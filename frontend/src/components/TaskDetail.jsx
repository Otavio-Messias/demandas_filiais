...
import { getStatus, getPriorityClass, formatDate, isOverdue, STATUSES } from '../utils';
...
export default function TaskDetail({ taskId, onClose, onUpdate }) {
  ...
  const isOwner = task && (task.assignee_id === user.id || task.created_by === user.id);
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', padding: '20px 24px 0', gap: 12 }}>
          <div style={{ flex: 1 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6, flexWrap: 'wrap' }}>
              ...
              {task.recurrence && <span style={{ fontSize: 11, fontWeight: 600, padding: '3px 8px', borderRadius: 20, background: '#f5f3ff', color: '#7c3aed' }}>🔁 {task.recurrence === 'diaria' ? 'Diária' : task.recurrence === 'semanal' ? 'Semanal' : 'Mensal'}</span>}
            </div>
        ...
        <div style={{ padding: '16px 24px 24px', display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 12, padding: 14, background: 'var(--surface-2)', borderRadius: 8 }}>
            ...
            <MetaItem label="Responsável">
              <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                ...
                {task.assignee_id === user.id && <span style={{ fontSize: 11, color: 'var(--text-3)' }}>(você)</span>}
              </div>
            </MetaItem>
          ...
          {task.checklist && task.checklist.length > 0 && (() => {
            ...
            return (
              <div>
                ...
                {!isOwner && <p style={{ fontSize: 11, color: 'var(--text-3)', marginTop: 6 }}>Somente o responsável pode marcar os itens.</p>}
              </div>
            );
          })()}
          ...
          <div>
            ...
            <div ref={commentsRef} style={{ maxHeight: 220, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 10 }}>
              {task.comments?.length === 0 && <p style={{ fontSize: 12, color: 'var(--text-3)', fontStyle: 'italic' }}>Nenhum comentário ainda.</p>}
              {task.comments?.map(c => (
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              ...
              <div style={{ flex: 1, display: 'flex', gap: 6 }}>
                <input className="form-input" placeholder="Adicionar comentário..." value={comment} onChange={e => setComment(e.target.value)} onKeyDown={e => e.key === 'Enter' && !e.shiftKey && sendComment()} />
